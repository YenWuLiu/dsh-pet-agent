...
class PetSprite {
  ...
  // 对话框定位（审批/设置共用）：优先贴身体命中区右上角，右侧放不下则翻到宠物左侧，
  // 最终按可视矩形夹取——宠物贴屏幕右缘时对话框绝不伸出屏幕。
  placeDialog(el) {
    const vis = this.visibleRect();
    const hr = this.hit.getBoundingClientRect();
    const rr = el.getBoundingClientRect();
    let left = hr.right + 6;
    if (left + rr.width > vis.x1 - 4) left = hr.left - rr.width - 6;
    left = Math.max(vis.x0 + 4, Math.min(left, vis.x1 - rr.width - 4));
    const top = Math.max(vis.y0 + 4, Math.min(hr.top + 6, vis.y1 - rr.height - 4));
  ...
  onMouseMove(e) {
    ...
    // 右键菜单/对话弹窗/权限确认框/设置弹窗开启：整窗保持可交互（悬停菜单项/点输入框/点按钮都不触发穿透翻转）；关闭后恢复命中区判定
    if (this.menuOpen || this.chatOpen || this.approvalOpen || this.settingsOpen) {
      this.setInteractive(true);
      return;
    }
  ...
  showChatFromMenu() {
    ...
    this.refreshVisibleRect(); // 对话弹窗按"屏幕工作区 ∩ 窗口"钳制（shared-core 读取该全局）
  ...
  showApprovalDialog(req) {
    ...
    const done = () => {
      ...
      this.approvalShowing = false;
      this.approvalOpen = false;
      if (!this.menuOpen && !this.chatOpen) this.setInteractive(false); // 无其它弹窗：恢复命中区穿透
    };
  ...
  showSettingsDialog() {
    ...
    autoRow.style.cssText = 'display:flex;align-items:center;gap:6px;cursor:pointer;margin-bottom:10px';
    ...
    const close = () => {
      root.remove();
      this.settingsOpen = false;
      if (!this.menuOpen && !this.chatOpen && !this.approvalOpen) this.setInteractive(false);
    };
    ...
    save.addEventListener('click', () => {
      ...
      fetch(BASE + '/settings', {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ autostart: autoBox.checked, model }),
      })
        .then((res) => res.json())
        .then((r) => {
          if (r && r.ok) {
...
function startLoops() {
  ...
  // 余额功能已按需求下线（菜单「查看余额」移除、/balance 在本宿主恒为 unsupported）：
  // 余额周期轮询与 1s 触发轮询一律不启动，避免无意义的周期请求。
  const anyBalanceEnabled = false;
