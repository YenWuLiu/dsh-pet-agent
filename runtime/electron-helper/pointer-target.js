 ...
 *
 * 本项目改动（仅 1 处，其余与上游 runtime/electron-helper/pointer-target.js 逐字一致）：
 *   `spriteHitRect` 补上 `bottomPad` —— 本项目渲染端的舞台被 `translateY(bottomPad)` 下移
 *   （窗口高 = 舞台高 + bottomPad），故画布顶边 = 窗口顶边 + margin + bottomPad。上游原版
 *   漏了这一项，兜底通道的命中区会比渲染端实际命中区高出约 stageH/12（462px 宠物约 22px），
 *   表现为宠物头顶多出一圈"可交互"（本该穿透到下层应用的像素被窗口吃掉）。
 *   守卫见 scripts/check-pointer-target.mjs —— 它把本函数与渲染端的 hitRect 公式逐项对齐。
 *
...
const CANVAS_H = 360;
const STAGE_W = 640;
/** 脚底线（与 shared 的 FEET_Y 一致）：画布底边到脚底的距离 = 舞台下方垫高的 bottomPad */
const FEET_Y = 330;

/**
 * 窗口矩形 → 宠物身体命中区的屏幕矩形（DIP）。
 * 窗口 = 宠物包围盒 + 四周各半只宠物的余量（renderer 的 WINDOW_MARGIN_RATIO = 0.5）：
 * 横向 `margin = round(width / 4)`、stageW = width − 2×margin；纵向从窗口顶边往下
 * margin + bottomPad 才是画布顶边。判定公式与渲染端 renderer.js 的 hitRect 同源，
...
function spriteHitRect(bounds) {
  ...
  return {
    ...
    top: bounds.y + margin + bottomPad + (HIT_BOX.y0 / CANVAS_H) * stageH,
    right: bounds.x + margin + (HIT_BOX.x1 / STAGE_W) * stageW,
    bottom: bounds.y + margin + bottomPad + (HIT_BOX.y1 / CANVAS_H) * stageH,
  };
}
