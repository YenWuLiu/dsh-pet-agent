 ...
 */
const { app, BrowserWindow, ipcMain, screen, protocol, Tray, Menu, nativeImage } = require('electron');
const path = require('node:path');
const { decideWindowIgnore, POINTER_POLL_MS } = require('./pointer-target.js');
...
/**
 * 托盘（内核桌宠扩展）。为什么必须有：桌宠窗口是**无边框、无任务栏项**的穿透小窗，
 * 用户没有关掉它的常规入口，一旦藏起来（或被 Win+D / 全屏应用盖住）就再也叫不回来；
 * 没有托盘时「退出桌宠」也只能去任务管理器杀进程。
 *
 * 左键单击 = 显示/隐藏桌宠；右键 = 极简菜单（显示/隐藏 · 退出桌宠）。
 * 刻意不用 tray.setContextMenu：Windows 上那会让**左键也弹菜单**，左键就收不回单击语义。
 */
let tray = null;
/** 托盘菜单对象（每次重建：显示/隐藏 是动态文案） */
let trayMenu = null;
/** 桌宠是否被托盘隐藏（隐藏用 hide 不用 close：close 会让窗口表清空） */
let petsHidden = false;
/** 已请求退出：抑制 window-all-closed 的重复退出与 kernel 退出后的兜底逻辑 */
let quitting = false;
...
/**
 * 每窗口当前穿透状态（true = 整窗点击穿透）。所有 setIgnoreMouseEvents 只经 setWindowIgnore()
 * 这一个出口（创建时初始化 + pet:set-interactive 翻转 + 兜底轮询），这里镜像真实状态，
 * 供冒烟断言/排查使用（Electron 无 isIgnoringMouseEvents 取值 API）。
 */
const windowIgnore = new Map();

/**
 * 每窗口「渲染端正拿着鼠标输入」标记（拖拽中 / 任意面板开着）。
 * 兜底轮询据此**否决**位置判定（见 pointer-target.js 的 decideWindowIgnore）：
 * 拖拽时宠物由 rAF 弹簧追赶光标、滞后于光标，窗口矩形只比身体大一圈余量——甩得快时
 * 光标落到矩形外，若按位置判成"窗外"就会翻回穿透，正在拖拽的 pointermove/pointerup 全断
 * （鼠标还按着，宠物却按旧速度飞出去）。这正是上游 0.2.10 → 0.2.11 修掉的回归。
 */
...
function createPetWindows() {
  ...
  const configUrl = process.env.DSH_PET_CONFIG_URL || 'http://127.0.0.1:7341/dsh-pet-7340/config';
  ...
  for (const pet of pets) {
    ...
    // ---- 点击穿透「兜底通道」（上游 issue #55 的修法）----
    // 渲染端那条通道只有一个入口：Electron 的 forward 低级鼠标钩子在 Windows 上会静默失效
    // （回调超时被系统摘掉、或被别的软件钩子干扰），失效后光标悬浮不触发手套光标、拖不动、
    // 点击与右键全无反应。这里每 60ms 按**真实光标位置**独立判定一次，两条通道共享
    // setWindowIgnore 这一份状态；渲染端正拿着输入（inputBusy）时兜底通道绝不翻回穿透。
    // 冒烟模式跳过：它按真实光标翻转 windowIgnore，会干扰冒烟对渲染端通道的断言。
    if (process.env.DSH_PET_SMOKE !== '1') {
      const poll = setInterval(() => {
        ...
        const next = decideWindowIgnore(bounds, screen.getCursorScreenPoint(), ignoring, inputBusy.get(win.id) === true);
        if (next !== ignoring) setWindowIgnore(win, next);
      }, POINTER_POLL_MS);
      win.on('closed', () => clearInterval(poll));
    }
    win
      // 绝对路径：开发流用 `electron <helperDir>` 起（app 路径 = helper 目录，相对名也能中），
      // 打包流里本文件是被 launcher.cjs require 进来的，app 路径 = resources/app——
      // 相对 'index.html' 会解析成 resources/app/index.html 而 ERR_FILE_NOT_FOUND。
...
/** 宿主 /dsh-pet-7340 前缀（与 renderer 的 BASE 同一推导：DSH_PET_CONFIG_URL 去掉 /config） */
function hostBase() {
  const url = process.env.DSH_PET_CONFIG_URL || 'http://127.0.0.1:7341/dsh-pet-7340/config';
  return url.replace(/\/config$/, '');
}
...
function localTrayIcon() {
  ...
  try {
    ...
    console.error('[dsh-pet-desktop-helper] tray icon load failed:', String(error && error.message ? error.message : error));
    return null;
  }
}

/** 宿主 /pic/ 兜底图标（本地 tray.png 缺失时；素材热替换后也跟得上） */
async function remoteTrayIcon() {
  for (const name of ['tray.png', 'notify-done.png']) {
    try {
      const res = await fetch(hostBase() + '/pic/' + name, { cache: 'no-store' });
      if (!res.ok) continue;
...
/** 托盘「显示/隐藏桌宠」：hide/show 而非 close，窗口、动画、会话状态全部原地保留 */
function setPetsHidden(hidden) {
  petsHidden = hidden;
  for (const win of windows.values()) {
    if (win.isDestroyed()) continue;
    if (hidden) win.hide();
    else win.showInactive(); // 不用 show()：桌宠始终置顶，抢焦点会打断用户正在打的字
  }
  refreshTrayMenu();
}

/**
 * 退出桌宠：先请求宿主做有界退出（内核 dispose 整棵树，作为副作用会 kill 掉本进程），
 * 再自行 app.quit() 兜底——宿主端口漂移或已先行退出时，光靠宿主杀不到我们。
 */
async function quitPet() {
  ...
  try {
    await fetch(hostBase() + '/quit', { method: 'POST' });
  } catch {
    /* 宿主已不可达：下面照常退出 */
  }
...
/**
 * 重建托盘菜单：**只留两个入口** —— 「显示/隐藏桌宠」与「退出桌宠」。
 * 托盘在这套设计里只干两件事：把宠物找回来、把它关掉（无边框穿透小窗没有任务栏项，
 * 所以必须有这两个入口）。其余动作都在宠物右键菜单里（对话 / 设置 / 碎碎念 / 回到初始位置），
 * 开机自启在「设置」卡里——菜单不再重复一遍，顺带也不再需要每次弹菜单前读宿主设置。
 * 左键单击 = 显示/隐藏开关（tray.on('click')），右键 = 弹本菜单。
 */
function refreshTrayMenu() {
  if (!tray || tray.isDestroyed()) return;
  const menu = Menu.buildFromTemplate([
    { label: petsHidden ? '显示桌宠' : '隐藏桌宠', click: () => setPetsHidden(!petsHidden) },
...
async function createTray() {
  if (tray) return;
  const icon = localTrayIcon() || (await remoteTrayIcon());
  if (!icon) {
    console.error('[dsh-pet-desktop-helper] no tray icon available, tray disabled');
    return;
  }
  try {
    tray = new Tray(icon);
  } catch (error) {
    console.error('[dsh-pet-desktop-helper] tray creation failed:', String(error && error.message ? error.message : error));
    tray = null;
    return;
  }
  tray.on('click', () => setPetsHidden(!petsHidden));
  tray.on('right-click', () => { if (trayMenu) tray.popUpContextMenu(trayMenu); });
  // 刻意不接 double-click：Windows 上双击会先补发两次 click（= 隐藏再显示，肉眼可见一闪），
  // 再叠一个双击动作；单击开关 + 右键菜单已经覆盖全部需求。
  refreshTrayMenu(); // 同步构建（不再依赖宿主设置就绪）
  console.error('[dsh-pet-desktop-helper] tray ready');
}

app.whenReady().then(() => {
  if (BRIDGE) {
    // 自定义 scheme 接住渲染端全部请求（配置/碎碎念/广播/素材）
  ...
  createPetWindows();
  // 托盘与窗口同批建立：托盘是桌宠唯一的"退出/找回"入口，越早越好
  void createTray();
  ...
  // 渲染端「正拿着鼠标输入」上报（拖拽中 / 面板开着）：兜底轮询据此绝不翻回穿透。
  // 只存标记、不直接翻窗口——唯一出口仍是 setWindowIgnore（两侧状态才不会被写乱）。
  ipcMain.on('pet:input-busy', (event, busy) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win || win.isDestroyed()) return;
    inputBusy.set(win.id, busy === true);
  });

  // 右键菜单「退出桌宠」：与托盘菜单同一条退出路径
  ipcMain.on('pet:quit-app', () => { void quitPet(); });
      ...
      try {
        ...
        if (first && !first.isDestroyed()) {
          const dump = await first.webContents.executeJavaScript(`(async () => ({
            ...
            // 双击自检（内核桌宠扩展）：在命中区派发 dblclick → 对话框应打开（chatOpen=true）
            // 前面的 drag 测试刚拖过宠：justDragged 100ms 内屏蔽双击，与菜单同等待 250ms
            dblclickSmoke: await (async function () {
              ...
              if (!hit) return null;
              hit.dispatchEvent(
                new MouseEvent('dblclick', { bubbles: true, cancelable: true, clientX: 400, clientY: 260, screenX: 400, screenY: 260 }),
              );
              await new Promise(function (resolve) {
                setTimeout(resolve, 80);
              });
              return { chatOpen: window.__dshPetDebug.chatOpen === true };
            })(),
            ...
            // 边界钳制自检（内核桌宠扩展）：在窗口最右缘（屏幕外区域）派发 contextmenu →
            // 菜单根面板右缘必须落回可视矩形内（__dshPetVisibleRect.x1）
            clampSmoke: await (async function () {
              ...
              if (!hit) return null;
              hit.dispatchEvent(
                new MouseEvent('contextmenu', { bubbles: true, cancelable: true, button: 2, clientX: window.innerWidth - 10, clientY: 300, screenX: window.innerWidth - 10, screenY: 300 }),
              );
              await new Promise(function (resolve) {
                setTimeout(resolve, 80);
              });
              var vis = window.__dshPetVisibleRect;
              var panel = document.querySelector('.dsh-pet-menu-column');
              if (!vis || !panel) return { vis: vis || null, panel: !!panel };
              var r = panel.getBoundingClientRect();
              return {
                visX1: vis.x1,
                panelRight: r.right,
                panelBottom: r.bottom,
                withinX: r.right <= vis.x1 + 1,
                withinY: r.bottom <= vis.y1 + 1,
              };
            })(),
            // 换片不变量（内核桌宠扩展）：任意时刻至少要有一层视频是全不透明的。
            // 换片是硬切（新层首帧上屏后一次性掀掉旧层），理论上覆盖度恒为 1；
            // 这里派发一次真实 click（走 280ms 去抖 → playOnce → switchTo 硬切），
            // 采样两层 opacity 的最小覆盖度：≥0.99 视为不变量成立（露背景 = 点一下整只宠物闪一下）。
            crossfadeCoverage: await (async function () {
              await new Promise(function (resolve) {
                setTimeout(resolve, 250);
              });
              var hit = document.querySelector('.pet-hit');
              if (!hit) return null;
              var vids = Array.prototype.slice.call(document.querySelectorAll('.pet-sprite video'));
              if (vids.length < 2) return { videos: vids.length };
              var op = function (v) {
                var o = parseFloat(getComputedStyle(v).opacity);
              ...
              var sample = function () {
                if (stop) return;
                var cover = 1 - (1 - op(vids[0])) * (1 - op(vids[1]));
                if (cover < minCover) {
                  minCover = cover;
                  minAt = { a: op(vids[0]), b: op(vids[1]) };
                }
                samples++;
              ...
              requestAnimationFrame(sample);
              hit.dispatchEvent(
                new MouseEvent('click', { bubbles: true, cancelable: true, clientX: 400, clientY: 260, screenX: 400, screenY: 260 }),
              );
              // 280ms 去抖 + 240ms 淡入 + 240ms 淡出 → 采 1.2s 足够覆盖整段换片
              await new Promise(function (resolve) {
                setTimeout(resolve, 1200);
              });
              stop = true;
              var srcs = vids.map(function (v) { return (v.src || '').split('/').pop(); });
              return {
                minCoverage: Math.round(minCover * 1000) / 1000,
                worstPair: minAt,
                samples: samples,
                ok: minCover >= 0.99,
                videos: srcs,
          ...
          // 对话面板深度自检（DSH_PET_CHAT_SMOKE=1）：真发一轮，校「面板常驻 + 逐字流式 +
          // 终态收尾」。前面 dblclickSmoke 已把面板打开，这里注入宿主转录 → 发送 → 等终帧。
          // 静默收尾（无重试按钮）与光标消失是这一步的核心断言。
          // 结论同时写文件（DSH_PET_SMOKE_OUT 同目录的 chat-smoke.json）：无管道环境下
          // stdout 可能拿不到，文件是唯一可靠的取回通道。
          if (process.env.DSH_PET_CHAT_SMOKE === '1') {
            let chatDump
            try {
              chatDump = await first.webContents.executeJavaScript(`(async function () {
              await new Promise(function (r) { setTimeout(r, 150); });
              var d = window.__dshPetDebug;
              if (!d || !d.chat) return { error: 'chat panel not mounted' };
              d.chat.restore([{ role: 'user', text: '（历史）你还记得我叫什么吗？' }, { role: 'pet', text: '（历史）你叫小明呀。' }]);
              var before = document.querySelectorAll('.dsh-pet-chat-msg').length;
              var run = d.chat.isFinished;
              var input = document.querySelector('.dsh-pet-chat-input');
              input.value = '你叫什么名字？';
              input.dispatchEvent(new Event('input', { bubbles: true }));
              document.querySelector('.dsh-pet-chat-send').dispatchEvent(new MouseEvent('click', { bubbles: true }));
              var streamingSeen = false;
              var streamLenSeen = 0;
              for (var i = 0; i < 200; i += 1) {
                await new Promise(function (r) { setTimeout(r, 50); });
                var live = document.querySelector('.dsh-pet-chat-msg.streaming span');
                if (live) {
                  streamingSeen = true;
                  streamLenSeen = Math.max(streamLenSeen, live.textContent.length);
                }
                if (run() === true) break;
              }
              await new Promise(function (r) { setTimeout(r, 250); });
              var rows = document.querySelectorAll('.dsh-pet-chat-msg');
              var last = rows[rows.length - 1];
              var dot = document.querySelector('.dsh-pet-chat-dot');
              var log = document.querySelector('.dsh-pet-chat-log');
              var status = document.querySelector('.dsh-pet-chat-status');
              return {
                mounted: true,
                busyNow: d.chat.isBusy(),
                finished: run(),
                msgCountBeforeSend: before,
                msgCountAfter: rows.length,
                lastText: last ? last.textContent : '',
                streamingSeen: streamingSeen,
                streamingMaxLen: streamLenSeen,
                caretGone: !document.querySelector('.dsh-pet-chat-msg.streaming'),
                retryRows: document.querySelectorAll('.dsh-pet-chat-retry').length,
                errRows: document.querySelectorAll('.dsh-pet-chat-err').length,
                statusText: status ? status.textContent : '',
                dotClass: dot ? dot.className : '',
                sendLabel: (document.querySelector('.dsh-pet-chat-send') || {}).textContent || '',
                chatOpen: d.chatOpen === true,
                interactive: d.chat === null ? false : d.interactive === true,
                log: log ? { h: log.scrollHeight, ch: log.clientHeight } : null,
                width: String(getComputedStyle(document.querySelector('.dsh-pet-chat')).width),
              };
            })()`)
            } catch (error) {
              chatDump = { error: String(error && error.message ? error.message : error) }
            }
            console.log('[dsh-pet-desktop-helper] smoke chat dump:', JSON.stringify(chatDump))
            try {
              writeFileSync(smokeOut.replace(/\.png$/i, '') + '.chat.json', JSON.stringify(chatDump, null, 2))
            } catch (error) {
              console.error('[dsh-pet-desktop-helper] chat dump write failed:', error)
            }
          }
          ...
          // 连拍自检（DSH_PET_SMOKE_BURST=<目录>）：派发一次真实 click 后按 ~30ms 连拍，
          // 供逐帧核对「待机→动作→待机」的入口/出口过渡（录屏只有 16fps、还带缩放，
          // 判断不了"重影"到底在哪一帧、是真轮廓错位还是贴图差）。
          if (process.env.DSH_PET_SMOKE_BURST) {
            const burstDir = process.env.DSH_PET_SMOKE_BURST;
            const burstFrames = Number(process.env.DSH_PET_SMOKE_BURST_FRAMES || 200);
            const burstGap = Number(process.env.DSH_PET_SMOKE_BURST_GAP_MS || 20);
            const { mkdirSync } = require('node:fs');
            mkdirSync(burstDir, { recursive: true });
            // 先让前面几个探针触发的动画跑完、回到待机
            await new Promise((r) => setTimeout(r, 6000));
            await first.webContents.executeJavaScript(
              "(() => { const h = document.querySelector('.pet-hit');" +
                " if (h) h.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true," +
                ' clientX: 400, clientY: 260, screenX: 400, screenY: 260 }));' +
                ' return !!h; })()',
            );
            const shots = [];
            for (let i = 0; i < burstFrames; i += 1) {
              const shot = await first.webContents.capturePage();
              const file = path.join(burstDir, 'b' + String(i).padStart(3, '0') + '.png');
              writeFileSync(file, shot.toPNG());
              shots.push(file);
              await new Promise((r) => setTimeout(r, burstGap));
            }
            console.log('[dsh-pet-desktop-helper] smoke burst:', shots.length, 'frames ->', burstDir);
          }
        }
      } catch (error) {
...
app.on('window-all-closed', () => {
  // 有托盘时窗口全关≠退出：托盘还活着，用户仍要能从菜单里把宠物叫回来。
  // 真正的退出走托盘菜单 → quitPet()（先让宿主 dispose，再 app.quit()）。
  if (tray && !tray.isDestroyed() && !quitting) return;
  app.quit();
});

// 退出前收摊：托盘图标不随进程退出会自动消失（Windows 会留一个幽灵图标直到鼠标划过）
app.on('will-quit', () => {
