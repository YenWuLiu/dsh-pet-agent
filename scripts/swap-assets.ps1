...
# --- 3.5 托盘图标（派生）---
# 托盘图标不是手绘素材，而是从角色表情派生的 32×32 PNG。放在这里自动重跑：
# 换了角色脸却忘了重生成托盘图标，任务栏上就会一直挂着上一个角色的脸。
$trayScript = Join-Path $PSScriptRoot 'make-tray-icon.py'
if (Test-Path $trayScript) {
  ...
  if ($trayCode -ne 0) { Write-Host '提示：托盘图标生成失败（不影响桌宠本体，托盘会回退到宿主图标）' }
} else {
  Write-Host '提示：找不到 scripts\make-tray-icon.py，跳过托盘图标生成'
}
