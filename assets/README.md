...
| 内容 | 状态 | 授权 |
|---|---|---|
| `webm/` | 本项目自制的动画（**现有 29 条**；纯黑底 HEVC 母版经 `key-video.py` 抠像 + 锚定） | 本项目自制，按仓库根 LICENSE（MIT） |
...
| `config.jsonc` | 由上游动画池配置改写；**只引用已出片的 29 条**（上一版那 92 个"待出片槽位名"已整体放弃） | 配置本身随代码 MIT |
...
2. **抠像 + 锚定**：`python scripts/key-video.py <src> <dst.webm> --anchor frame0`
   ...
   > **纯黑底片（本批 29 条）必须显式给 `--thresh 10`**：脚本默认 36 会把角色的暗部
   > （深藏青裙/发）也当成背景吃掉。本批实测 6~10 是平台期（背景占比稳定 0.814、
   > 角色是单一连通块且零孔洞），10 再往上就开始咬断角色边缘。
   >
   > **片里有独立的气泡/道具/特效时再加 `--keep-min-blob 80`**：默认规则是"只保留最大
   > 连通域"（用来甩掉即梦的「AI生成」水印），但与角色不连通的**对话气泡、掉在地上的
   > 手柄、鲸鱼虚影**会被整块抹掉——实测 `深度思考碎碎念` 的三个中文气泡各约 13.6k px、
   > `玩游戏气急败坏` 的手柄约 5.3k px。该参数只放宽"保留哪些块"，
   > **锚点与缩放仍然只认角色本体（最大块）**，气泡不会把角色顶小。
