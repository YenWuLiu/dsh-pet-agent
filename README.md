...
`docs/动画生成清单.md`）。`assets/config.jsonc` 里当前的动画名都是**占位名**。

| 内容 | 状态 | 授权 |
|---|---|---|
| `assets/webm/` | 本项目自制动画（**现有 14 条**，逐条出片） | 自制，随代码 MIT |
...
**当前进度（2026-09-22）**：`assets/webm/` 有 **14 条**自制动画——手抠带 alpha 的 MOV 母版
（752×560 / 864×496 / 560×752，30fps）经 `scripts/normalize-webm.py` 归一化成 640×360
VP9-Alpha：待机 1 · 移动 1 · 小动作 4 · 玩耍 7 · 吃什么 1。`assets/config.jsonc` **只引用这 14 条**，
所以 `scripts/check-assets.ps1` 的「缺 webm」已清零（引用数 106 → 14）；还没出片的 92 个槽位名
集中列在 config 文件末尾的注释块里，出片后把名字挪进对应池子即可。
剩下唯一的红是上一轮就有的**两条锚点超差**（`整体换装试色` 脚底 −19px、`小提琴演奏` 脚底 −11px）：
这两条片内人物纵向位移大（最低点比首帧脚底低 49/35px），而画布 `FEET_Y=330` 下方只有 30px 余量，
脚本选择「保住内容」就必然让首帧悬空——要么重出，要么接受。
...
| `balanceEnabled` | **false** | 余额查询已去掉（宿主无 `/balance` 路由、渲染端不轮询不弹气泡）。该字段与 `eventsRefreshSec.balance` 都只是**兼容旧配置的必填项**，不再有任何行为；6 条余额档位动画还没出片，所以暂时也不在「动作」菜单里（名字见 `assets/config.jsonc` 末尾名单） |

右键菜单：对话 / 设置 / 回到初始位置 / 退出桌宠 + 「动作」（**待机 1 · 移动 1 · 小动作 4 ·
玩耍 7 · 吃什么 1**，全是已出片的，可直接点播）。**「碎碎念」这一项暂时不显示**——它要先请宿主
