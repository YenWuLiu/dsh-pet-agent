...
**当前进度（2026-09-23）**：`assets/webm/` 有 **29 条**自制动画。母版是重出的一批**纯黑底 HEVC**
（无 alpha，752×560 / 864×496 / 560×752，30fps，10.07~10.09s），经 `scripts/key-video.py` 抠像
（边界洪水填充去黑底，`--thresh 10`；带独立气泡/道具的片再加 `--keep-min-blob 80`）
并锚定成 640×360 VP9-Alpha：
待机 1 · 转向 1 · 点击回应 2 · 移动 1 · 小动作 9 · 玩耍 12 · 吃什么 2 · 文字 1。
`assets/config.jsonc` **只引用这 29 条**，`scripts/check-assets.ps1` **三项全绿**：
配置共引用 29 个动画、0 个缺文件、全部 640×360 带 alpha，锚点契约 28 条全部达标
（最大偏差 2px，容差 ±8px）。上一版那 92 个"待出片槽位名"已按「以新素材为准」整体放弃。

> **本批相对上一版的变化**：素材从 14 条增加到 29 条。其中 `东张西望`（转向）、
> `点击回应-害羞惊讶` / `点击回应-开心跃动`（点击回应）、`吃Token` / `吹气球` / `扑克魔术` /
> `玩水枪` / `被吓一跳` / `趴地熟睡` / `跪坐干饭` / `原地向左奔跑` / `鲸鱼吐泡泡`，
> 以及后加的 `原地敲击桌面互动` / `用鲸鱼尾巴拍打地面` / `玩游戏气急败坏` / `鲸鱼现世` /
> `深度思考碎碎念` 都是新片。
> 因此 `turn` / `clicks` 两个池子**第一次被填上**（`animationWeights.turn` 从 0 给回 5），
> 宠物现在会真的转身（`facing` 会翻转）——`深度思考碎碎念` 带中文对话气泡，是唯一必须
> `noMirror` 的动画，已单独归进「文字」类。上一版两条锚点超差（`整体换装试色` 脚底 −19px、
...
移动 1 · 小动作 9 · 玩耍 12 · 吃什么 2 · 文字 1**，全是已出片的，可直接点播）。**「碎碎念」这一项暂时不显示**——它要先请宿主
