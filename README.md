...
**当前进度（2026-09-23）**：`assets/webm/` 有 **38 条**自制动画。母版是 `mov\` 下那批
...
> ⚠ **一条踩过的坑（2026-09-23）**：`mov\convert-mov-to-webm.bat` 里原本带
> `colorkey=0x00FF00:0.1:0.1`（绿幕键），而母版是**手扣好、自带 alpha** 的 MOV。`colorkey`
> 只按 RGB 工作，会**丢弃输入 alpha、按 RGB 重新生成一张**——键色 `#00FF00` 在这些母版的
> **透明黑底**上匹配不到任何像素，于是新 alpha 全是 255，**成片完全不透明**（容器还照旧带
> `alpha_mode:1`，实机才发现没有透明通道）。实测同一份母版首帧透明占比
> **0.8196 → 0.0000**。所以 `mov\output\` 里那批 webm 一律**不能**当素材来源；bat 已修
> （去掉 colorkey、补 `-an`）。素材改为**直接从 MOV 母版归一化**，既拿到真 alpha，
> 又少一代有损编码。编码用 **CRF 15**（2026-09-23 从 20 下调，实测显示尺寸 +1.19 dB），
> 38 条合计 **57.4 MB**（上一版从被抠坏的 webm 转的是 70.6 MB —— 更小且更清晰）。
