...
**桌宠只是她的外壳**：一只角色站在桌面右上角，双击就说话。**动画由本项目自制 —— 本仓库
不附带任何动画文件**（`assets/webm/` 是空目录），见 [素材与授权](#素材与授权)。
...
本项目规划中的动画清单（12 制，见 `docs/动画生成清单.md`）：待机 1 · 转向 1 · 拖拽 1 ·
移动 1 · 点击回应 3 · 小动作 5，外加「玩耍」池 3 条。**当前一条都还没有**，
`scripts/check-assets.ps1` 会如实报「缺 webm」——这是预期状态，出片后逐条补齐。
...
| `balanceEnabled` | **false** | 余额查询已去掉（宿主无 `/balance` 路由、渲染端不轮询不弹气泡）。该字段与 `eventsRefreshSec.balance` 都只是**兼容旧配置的必填项**，不再有任何行为；6 条余额档位动画仍可在「动作」菜单手动点播 |

菜单点播也跟着放开了：右键 →「碎碎念」立即生成一句；菜单项就是 对话 / 设置 / 碎碎念 /
回到初始位置 / 退出桌宠 + 「动作」（含**余额档位**与**碎碎念**两档事件池，共 9 条，
可直接点播）。
...
- **代码**: [MIT](LICENSE)
- **动画**（`assets/webm/`）: 本项目自制，随代码按 MIT 处理（**本仓库不附带任何动画文件**）
