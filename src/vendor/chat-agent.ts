 ...
 * summarize contract). `onEvent` projects progress into a small UI-safe
 * vocabulary — text/reasoning deltas, tool calls and results, turn ends —
 * scoped to this instance's session only.
 *
 * Two event sources feed that vocabulary, because the kernel split them in
 * 0.1.5-rc.x (this file used to read a `session/event` named `assistant/chunk`,
 * which no longer exists):
 *   - the durable session log (`session/event`) for turn/tool/approval
 *     boundaries, and for the step's assembled text when nothing streamed;
 *   - the process-local `agent/assistant-stream` publication for live deltas.
 *     Durable assistant events now carry the whole compacted `stream` array at
 *     settlement time, so the log alone can only replay text *after* a step
 *     finishes — reading it for deltas would kill the typing effect.
 *
...
import { createUserMessage, joinAssistantStreamText } from '@deepseek-ai/dsh-llm'
...
/** Map one live model chunk to the public vocabulary; undefined for blocks/usage/other. */
function mapChunk(chunk: StreamChunk): ChatAgentEvent | undefined {
...
export async function createChatAgent(ctx: Context, options: ChatAgentOptions = {}): Promise<ChatAgent> {
  ...
  /** Whether this turn already streamed live text; gates the settled-message fallback. */
  let sawLiveChunks = false
  const ownSessionId = String(agent.session.id)

  // Live deltas: process-local publication from the agent loop (see the module
  // doc). `agent/assistant-stream` is broadcast tree-wide and scope-filtered per
  // agent, so compare session ids rather than object identity — a wrapped Agent
  // would defeat `===`.
  const disposeStream = ctx.on('agent/assistant-stream', (payload) => {
    if (String(payload.agent.session.id) !== ownSessionId) return
    const frame = payload.frame
    if (frame.type !== 'chunk') return // start/end carry no user-visible text
    const mapped = mapChunk(frame.chunk)
  ...
  const disposeEvents = ctx.on('session/event', (session, event) => {
    ...
    if (event.type === 'turn/start') sawLiveChunks = false
    // Fallback for a host that publishes no live frames: emit the settled step's
    // text exactly once, from the durable compacted stream. Skipped when deltas
    // already streamed, or the bubble would show the reply twice.
    if (event.type === 'assistant/message') {
      if (sawLiveChunks) return
      const stream = event.data.stream
      if (Array.isArray(stream) && stream.length > 0) {
        const text = joinAssistantStreamText(stream)
